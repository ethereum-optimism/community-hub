...
- *For the time period between rounds 1 - 5, we included addresses who interacted with L1 Gitcoin contracts, sent legacy `ExecuteSubscription` calls, or appeared in [Gitcoin’s donor API](https://docs.gitcoin.co/mk_rest_api/).*
- *For the period between rounds 6 - 13 (present), we included donor addresses in the L1 Gitcoin ‘BulkCheckout’ contract transaction logs.*
